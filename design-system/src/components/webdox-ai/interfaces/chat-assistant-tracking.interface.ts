import type { AssistantAIServiceType, AssistantLayoutTabType } from './chat-assistant.interface';
...
import type { ChatBotFeedbackPayload, CustomPrompt } from './webdox-ai.interfaces';
import type { PageScopeType } from '../../../interfaces/common.interfaces';
...
export type AssistantTrackingSendQueryPayloadType = Pick<
  AssistantTrackingPayloadType,
  'document' | 'pageScope' | 'message' | 'selectedTab' | 'origin'
>;

export type AssistantTrackingCloseAssistantPayloadType = Pick<
  AssistantTrackingPayloadType,
  'document' | 'pageScope'
>;

export type AssistantTabChangeAssistantPayloadType = Pick<
  AssistantTrackingPayloadType,
  'document' | 'pageScope' | 'selectedTab'
>;

export type AssistantCopyToClipboardPayloadType = Pick<
  AssistantTrackingPayloadType,
  'document' | 'pageScope' | 'message'
>;

export type AssistantFeedbackPayloadType = Pick<
  AssistantTrackingPayloadType,
  'document' | 'pageScope' | 'message' | 'feedback'
>;

export type AssistantClickSuggestionsPayloadType = Pick<
  AssistantTrackingPayloadType,
  'document' | 'pageScope'
>;

export type AssistantSelectSuggestionPayloadType = Pick<
  AssistantTrackingPayloadType,
  'document' | 'pageScope' | 'answerVariant' | 'message'
>;

export type AssistantChangeContractKindPayloadType = Pick<
  AssistantTrackingPayloadType,
  'document' | 'pageScope' | 'message'
>;

export type AssistantChangeMetadataPayloadType = Pick<
  AssistantTrackingPayloadType,
  'document' | 'pageScope' | 'message'
>;

export type SignatureAITrackingPayloadType = Pick<
  AssistantTrackingPayloadType,
  'document' | 'pageScope'
>;
