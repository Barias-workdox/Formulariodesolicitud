...
} from './chat-bot-component.interface';
import type { ChatConversationAction } from './chat-bot-conversation-state.interface';
import type {
  ChatBotAnswerTypeV2,
...
import type { LegalWhisperAnswerType } from './legal-whisper.interfaces';
import type {
...
import type { SelectProps } from '@components/select/next/select.interfaces';
...
export type UserChatMessageType = MessageType &
  Pick<ChatBotAnswerTypeV2, 'id' | 'uuid'> & {
    kind: 'question';
    value?: ChatBotAnswerTypeV2['value'];
    layoutKind?: MessageLayoutKindType;
  };

export type ChatBotChatMessageType = WithTestId &
  MessageType &
  Pick<ChatBotAnswerTypeV2, 'id' | 'uuid' | 'staticContent'> &
  Pick<LegalWhisperAnswerType, 'quotes'> &
  Pick<
