import type { ContentTypes } from './filter-group-factory.constants';
import type { FilterProps } from '@components/filter/filter.interfaces';
import type {
  Item,
  ListFactoryPaginationProps,
} from '@components/list-factory/list-factory.interfaces';

type IdentificationType = 'chile-rut';

export type FilterFactoryContentTypeVariant =
  | 'text'
  | 'number'
  | 'email'
  | 'boolean'
...
export type OnFilterChangeParams<T extends string = string> = {
  ...
  [K in ContentTypes]: { type: K } & FilterFactoryRawValuesMap[K];
}[ContentTypes];
