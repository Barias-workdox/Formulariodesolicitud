...
describe('PhoneInput - test', () => {
  ...
  it('should handle undefined country', () => {
    renderComponent({ country: undefined });

    expect(screen.getByTestId(`${defaultProps['data-testid']}--tel-input`)).toBeInTheDocument();

    expect(
      screen.getByTestId(
        `${defaultProps['data-testid']}__country-select__empty-value-flag--wrapper`,
      ),
    ).toBeInTheDocument();

    expect(
      screen.getByTestId(`${defaultProps['data-testid']}__country-select__empty-value-flag--icon`),
    ).toBeInTheDocument();
  });
});
