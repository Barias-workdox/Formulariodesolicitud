...
import type { AvatarListItemProps } from '@components/list/components/avatar-list-item';
import type { ListItemProps } from '@components/list/components/list-item';
...
export type ListItemFactoryProps = WithTestId &
  Omit<Item, 'id'> &
  Pick<ListItemProps, 'overrides' | 'aiGenerated'> & {
    onClick(): void;
  };
...
export type ListFactoryProps = WithTestId & {
  ...
  onItemClick(params: { item: Item; multi?: boolean }): void;
} & (
    | {
        isFiltrable: true;
        searchValue: string;
