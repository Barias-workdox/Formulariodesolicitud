...
import type { ActiveColumn, ColumnConfig, DataTableProps } from '../data-table.interfaces';
...
type TestActiveColumn = ActiveColumn<keyof DataType | ExtraColumnsIds>;

type TestTableProps = DataTableProps<DataType, ExtraColumnsIds>;

type TestOnChangeParams = Parameters<TestTableProps['onChange']>[0];
...
const activeColumns: TestActiveColumn[] = (['id', 'name'] as const).map((id) => ({ id }));

const initialColumnsConfig: AllColumnsConfig[] = activeColumns.map(
  (column) => allColumnsConfig.find(({ id }) => id === column.id)!,
);
...
const data = getTableData({ rawData, activeColumns, allColumnsConfig, actionCell: ActionCell });
...
describe('DataTable', () => {
  ...
  it('shows a new column correctly', async () => {
    ...
    const labelToActivate = 'Age';

    // Click on the "Age" menu item
    const ageMenuItem = screen.getByText(labelToActivate);
    ...
    const columnConfigToActivate = allColumnsConfig.find(({ label }) => label === labelToActivate);

    const newActiveColumn: TestActiveColumn | undefined = columnConfigToActivate
      ? {
          id: columnConfigToActivate.id,
        }
      : undefined;

    // We add the clicked column by its label
    const activeColumnsUpdated: TestActiveColumn[] = newActiveColumn
      ? [...activeColumns, newActiveColumn]
      : activeColumns;

    const columnsConfigUpdated: AllColumnsConfig[] = activeColumnsUpdated.map(
      (column) => allColumnsConfig.find(({ id }) => id === column.id)!,
    );

    const propsUpdated: TestOnChangeParams = {
      ...props,
      data: tempData,
      activeColumns: activeColumnsUpdated,
  ...
  it('hides a column correctly', async () => {
    ...
    const columnConfigToHide = allColumnsConfig.find(({ label }) => label === columnToHideLabel);

    // Update columnsConfig to remove the hidden column
    const activeColumnsUpdated: TestActiveColumn[] = activeColumns.filter(
      ({ id }) => id !== columnConfigToHide?.id,
    );
  ...
  it('sorts ascending correctly', async () => {
    ...
    const propsUpdated: TestOnChangeParams = {
      ...
      orderBy: 'name',
      orderDirection: 'asc',
      columnsConfig: initialColumnsConfig,
    };
