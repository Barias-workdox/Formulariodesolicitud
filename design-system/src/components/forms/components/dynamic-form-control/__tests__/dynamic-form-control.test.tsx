...
import type { DynamicFormControlProps } from '../dynamic-form-control';
...
const defaultProps = {
  ...
  countryCode: 'CHL',
} as const satisfies DynamicFormControlProps<ControlKindType>;

const mockOnSubmit = testHelpers.fn();

/** Utility to render component quickly with default props and allows overrides of every prop */
const renderComponent = (props?: Partial<DynamicFormControlProps<ControlKindType>>): RenderType => {
  return render(
...
describe('dynamic-form-control - tests', () => {
  afterEach(() => {
  ...
  it('should focus the rendered control when the form is submitted and there is an error', async () => {
    render(
      <FormProviderControlWrapper
        onSubmit={mockOnSubmit}
        schema={zod.object({
          dynamicFormControl: zod.string().min(1, 'Required'),
        })}
        resolverType="zod"
      >
        <DynamicFormControl {...defaultProps} />
        <Button type="submit">Submit</Button>
      </FormProviderControlWrapper>,
    );
