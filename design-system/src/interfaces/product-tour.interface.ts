import type { NativeLocaleType } from '../components/utils/i18n/i18n.interface';
import type { SnakeToCamelCaseNested } from '../utils/type.util';
...
export type ProductTourSettingsType = IntercomMessengerAttributesType &
  IntercomDataType &
  ProductTourDynamicType;
...
export type IntercomType = {
  ...
  Intercom(
    action: ProductTourActionType,
    context?: ProductTourTriggerEventParamsType['context'],
    metadata?: ProductTourTriggerEventParamsType['metadata'],
  ): void;
};

export interface ProductTourSettingsCompanyParamType extends Omit<
  SnakeToCamelCaseNested<IntercomCompanyType>,
  'createdAt'
> {
  createdAt?: string;
}
 ...
 */
export interface ProductTourSettingsParamsType extends Omit<
  ProductTourSettingsBaseParamsType,
  'customLauncherSelector' | 'hideDefaultLauncher'
> {
  ...
  user: Omit<
    ProductTourSettingsBaseDataParamsType,
    'createdAt' | 'languageOverride' | 'lastRequestAt' | 'company' | 'companies'
  > & {
    createdAt: string;
