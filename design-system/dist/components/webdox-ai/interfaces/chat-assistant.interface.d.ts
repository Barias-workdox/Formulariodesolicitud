...
import { ChatConversationAction, ChatConversationState } from './chat-bot-conversation-state.interface';
import { ChatBotAnswerTypeV2, ChatBotConversationTypeV2, GenerativeAnswerType, GenerativeTextValueType } from './chat-bot.interfaces';
import { AnswerReference, ChatBotMessageTypePropsV2 } from './chat-message.interface';
import { DocumentMetadata } from './document-metadata.interfaces';
import { ChatBotQuestion, CreateQuestionOptionsType, GenerateContractSummaryOptionsType, WithOnSuccess } from './webdox-ai.interfaces';
import { AssistantLayoutProps } from '../components';
import { PlanType } from './webdox-ia-plans.interfaces';
...
export type AssistantLayoutTabType = (typeof assistantConstants.assistantTabs)[keyof typeof assistantConstants.assistantTabs];
...
export type ChatAssistantContextValuesType = {
    ...
    onTranslateText(params: {
        text: string;
        targetLanguage: LocaleOption;
    }, options?: WithOnSuccess): void;
    /** Used to explain the selected text. */
    onExplainText?(params: {
        text: string;
    }, options?: WithOnSuccess): void;
    /** Used to retry a question. */
    onRetryQuestion(data: {
        id: string;
...
export type AssistantTypeProps = Omit<ChatBotMessageTypePropsV2, 'data-testid' | 'disabled'> & Pick<ChatAssistantValuesType, 'isPreparingConversation' | 'isPreparingDocument' | 'isConversationEmpty' | 'isPreparingMetadata'> & Pick<ChatAssistantContextValuesType, 'contractKind' | 'highlightedText' | 'onDataExtractionHighlight'> & {
    'data-testid': string;
    selectedTab: AssistantLayoutTabType;
