...
import { DocumentType, UserType } from '../../interfaces/common.interfaces';
import { StepOverrides } from 'baseui/progress-steps';
/** @deprecated Use the `Timeline` API instead */
export type ActivityType = 'flag' | 'view' | 'upload' | 'download' | 'comment' | 'document' | 'request' | 'flow' | 'warning' | 'approved' | 'rejected' | 'new-user' | 'new-document' | 'delete'
...
/** @deprecated Use the `Timeline` API instead */
export type IconConfigurationType = Pick<BackgroundIconProps, 'iconColor' | 'Icon' | 'backgroundColor'>;
/** @deprecated Use the `Timeline` API instead */
