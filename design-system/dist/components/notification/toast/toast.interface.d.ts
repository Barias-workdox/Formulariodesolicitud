...
import { PLACEMENT, ToastProps as BaseToastProps, KindType, NotificationType, PlacementType, ToasterOverrides, ToasterSharedStylePropsArg } from 'baseui/toast';
import { ToasterContainerContextProps } from './toaster-container-context/toaster-container-context.interface';
import { CarbonIconType } from '@carbon/icons-react/lib/CarbonIcon';
import { DesignSystemColorType } from '../../../themes';
export type { KindType, NotificationType, ToasterOverrides, ToasterSharedStylePropsArg, PlacementType, };
...
export type ToastProps = Omit<ToastBodyProps, 'type'> & Omit<BaseToastProps, 'children' | 'notificationType'> & {
...
export type ToasterType = {
    ...
    positive({ placement, zIndex, marginX, marginY, duration, ...rest }: ToasterKindParams): Promise<React.Key | null>;
    negative({ placement, zIndex, marginX, marginY, duration, ...rest }: ToasterKindParams): Promise<React.Key | null>;
    warning({ placement, zIndex, marginX, marginY, duration, ...rest }: ToasterKindParams): Promise<React.Key | null>;
    info({ placement, zIndex, marginX, marginY, duration, ...rest }: ToasterKindParams): Promise<React.Key | null>;
